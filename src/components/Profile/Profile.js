...
import { Card, Col, Container, FormControl, Image, Nav, Row, Tab } from 'react-bootstrap';
...
const Profile = () => {
    ...
    const handleDeleteUser = id => {
        ...
        if (!disabled) {
            ...
            image.map(item => {
                fetch(`https://photo-album-server.herokuapp.com/image/${item._id}`, {
                    method: 'DELETE'
                })
                    .then(res => res.json())
            })
            ...
            fetch(`https://photo-album-server.herokuapp.com/users/${id}`, {
                method: 'DELETE'
            })
                .then(res => res.json())
                            ...
                            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                {
                                    profile?.avatar ?
                                        ...
                                        <Image style={{ height: '100px', width: '100px' }} roundedCircle src={avatarImage} alt="" />
                                }
                            </div>
